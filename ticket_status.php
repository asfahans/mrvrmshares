...
$status_code = '';
if (!empty($ticketID)) {
    // Prepare and execute a SQL query to retrieve the ticket status
    $sql = "SELECT ticket_status FROM feedback WHERE ticket_no = ?";
...
<head>
<meta charset="utf-8">
<title>VRM Share Broking Pvt. Ltd. | Ticket Status</title>
<meta name="description" content="Check customer support ticket status at VRM Share Broking Private Limited.">
<!-- Stylesheets -->
...
<div class="page-wrapper">
    ...
    <main id="main-content">
        ...
        <div id="final-content">
            <!-- Ticket Status Display Section -->
            <section class="about-section" aria-label="Support Ticket Status Details">
                <div class="auto-container">
                    <div class="row clearfix">
                        <div class="col-lg-8 col-md-10 col-sm-12 mx-auto">
                            <div class="inner">
                                <div class="sec-title mb-4">
                                    <div class="upper-text">SUPPORT TICKET TRACKER</div>
                                    <h2>Ticket Status Details</h2>
                                </div>
                                <?php if (!empty($status_code)) { ?>
                                <div class="card p-4 border shadow-sm">
                                    <div class="card-body">
                                        <p class="mb-3" style="font-size: 1.1rem;"><strong>Ticket Reference ID:</strong> <span class="badge badge-secondary p-2 ml-2" style="font-size: 1rem;"><?= htmlspecialchars($ticketID) ?></span></p>
                                        <p class="mb-0" style="font-size: 1.1rem;"><strong>Current Status:</strong> 
                                            <?php 
                                            if ($status_code == 'Received') {
                                                echo '<span class="status-badge-received ml-2">Received</span>';
                                            } elseif ($status_code == 'In Process') {
                                                echo '<span class="status-badge-process ml-2">In Process</span>';
                                            } else {
                                                echo '<span class="status-badge-closed ml-2">Closed</span>';
                                            }
                                            ?>
                                        </p>
                                    </div>
                                ...
                                <?php } else { ?>
                                <div class="alert alert-warning p-4 role="alert" style="font-size: 1.05rem;">
                                    <strong>No Ticket Found:</strong> Ticket ID "<?= htmlspecialchars($ticketID) ?>" was not found or an invalid request occurred. Please verify your Ticket ID or submit a new inquiry via <a href="feedback.php" style="color: #0056b3; text-decoration: underline; font-weight: bold;">Support &amp; Feedback</a>.
                                </div>
                                <?php } ?>
                                <div class="mt-4 text-center">
                                    <a href="feedback.php" class="btn btn-outline-primary px-4 py-2" style="font-weight: 600;">Back to Support Page</a>
                                </div>
                            </div>
                        </div>
                    </div>
